...
function MenuPoses_SetPose(pose, modelType, destroy)
    ...
    if modelType == "exo" then 
        local exoVariant = options.exoVariant
        if exoVariant == kExoVariant.nebula then
            model.renderModel:SetMaterialParameter("nebula", 1.0)
    ...
    if modelType == "rifle" then
        local rifleVariant = options.rifleVariant
    ...
    if modelType == "pistol" then
        local pistolVariant = options.pistolVariant
        if pistolVariant == kPistolVariant.nebula then
            model.renderModel:SetMaterialParameter("nebula", 1.0)
            pistolVariant = kPistolVariant.anniv
        end
        model.renderModel:SetMaterialParameter("textureIndex", pistolVariant - 1)
    end

    if modelType == "axe" then
        local axeVariant = options.axeVariant
        if axeVariant == kAxeVariant.nebula then
            model.renderModel:SetMaterialParameter("nebula", 1.0)
            axeVariant = kAxeVariant.anniv
        end
        model.renderModel:SetMaterialParameter("textureIndex", axeVariant - 1)
    end
    
    if modelType == "marine" or modelType == "decal" then
        if options.marineVariant == kMarineVariant.nebula then
            model.renderModel:SetMaterialParameter("nebula", 1.0)
        end
    end
...
function MenuPoses:SetCoordsOffset(name)    --McG: FIXME This is duplicate code 
...
local axeOffsetY = -0.265
local axeOffsetZ = 0.75

local pistolOffsetY = -0.015
local pistolOffsetZ = 1.185
...
local shotgunOffsetZ = 1.4
    ...
    if coords then
        ...
        elseif name == "axe" then
            angles.yaw = (modelYaw + 0.45) * math.pi
            angles.pitch = 0.5
            angles.roll = 1.25
            coords = angles:GetCoords()
            coords.origin = coords.origin + Vector(0, axeOffsetY, axeOffsetZ)

        elseif name == "pistol" then
            angles.yaw = (modelYaw + 0.75) * math.pi
            angles.pitch = -0.5 * math.pi
            coords = angles:GetCoords()
            coords.origin = coords.origin + Vector(0, pistolOffsetY, pistolOffsetZ)
